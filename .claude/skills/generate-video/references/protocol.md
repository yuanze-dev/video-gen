...
一键生产的成功 `result` 包含 `scenes`、`productionGuard`、`media`、`audio`、`preparedConfig` 与 `lock`。标准交付必须看到 `productionGuard.policy="standard"`、`passed=true`，且 `scenes.opening/content/ending` 都存在；CLI 会在发布输出前对 `media` 的 H.264/MP4、宽高、fps、总时长和应有音轨做失败关闭验证。
