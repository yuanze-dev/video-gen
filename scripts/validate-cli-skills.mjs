...
requirePattern(skillText, /默认走 `sound-effect`[\s\S]*显式传 `--audio-kind music`/, "Skill 固定项目级音效默认且 Music 仅显式启用");
requirePattern(skillText, /不得写入 `opening\.countdown`[\s\S]*`opening\.curtain`[\s\S]*`ending`/, "Skill 禁止无请求覆盖标准片头片尾");
requirePattern(skillText, /PRODUCTION_GUARD_FAILED[\s\S]*--allow-custom-structure/, "Skill 记录标准成片护栏与显式越过边界");
requirePattern(skillText, /productionGuard\.policy[\s\S]*scenes[\s\S]*media/, "Skill 要求验证成片终态而非仅看路径");
requirePattern(skillText, /空间 \+ 持续物理声源[\s\S]*不复制台词/, "Skill 以环境声源构造音效提示词");
requirePattern(skillText, /`required`[\s\S]*空 BGM[\s\S]*内置模板 BGM[\s\S]*`--replace-bgm`/, "Skill 记录 required 对内置 fallback 的生成语义");
...
requirePattern(workflowsText, /Electron 客户端[\s\S]*同一本地事务[\s\S]*Codex[\s\S]*Claude/, "Electron 安装事务自动部署用户级 Skill");
requirePattern(protocolText, /OUTPUT_EXISTS[\s\S]*--force/, "batch 不匹配产物的覆盖规则");
requirePattern(protocolText, /PRODUCTION_GUARD_FAILED[\s\S]*退出码 `3`/, "机器协议声明标准成片护栏错误");
...
requirePattern(readmeText, /默认使用 `sound-effect`[\s\S]*显式传 `--audio-kind music`/, "README 固定项目级音效默认且 Music 仅显式启用");
